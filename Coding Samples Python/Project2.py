"""
Submission for Project 2, Election Night Revisted - The Real Deal.

Program conducts Australian elections with full and optional preferential voting.
Requires two text file inputs; one candidates list and one list of submitted ballot papers.
Optional preferential voting requires a third True value input.
...
def getCandidates(f):
    try:
        ...
        with open(f) as file:
            for line in file:
                candidatelist += line.strip().split('\n')
